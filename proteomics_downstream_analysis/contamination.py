...
class ContaminationAnalysis:
    ...
    def __init__(self):
        filepath = "../data/contamination/contam_panel.xlsx"
    ...
    def outlier_plot(
        ...
        if kind == "zscore":
            ...
            output = [
                self.compute_contamination_outlier(data[["Genes", i]], contam_type)[:2]
                for i in groups
            ]
    ...
    def compute_rbc_total_ratio(self, data, contam_type="RBC"):
    ...
    def compute_contamination_outlier(self, data, contam_type="RBC"):
        # sort data
        data = self.sort_by_column_names(data)

        # calculate rbc to total protein ratio
        rbc_total_ratio = self.compute_rbc_total_ratio(data, contam_type)
    ...
    def contamination_outlier(
        ...
        if experimental:
            master_mask = np.array([], dtype=bool)
            for i in data.select_dtypes(float).columns.unique():
                _, _, mask = self.compute_contamination_outlier(
                    data[["Genes", i]], contam_type
                )
                master_mask = np.concatenate((master_mask, mask))

        else:
            _, _, master_mask = self.compute_contamination_outlier(data, contam_type)
    ...
    def contamination_outlier_plot(self, data, experimental=True, type="RBC"):
        ...
        if experimental:
            ...
            for group, axes in zip(groups, ax.flat):
                rbc_total_ratio, upper_lim, _ = self.compute_contamination_outlier(
                    data[group], type
                )
    ...
    def missing_values_outlier_plot(self, data, experimental):
        ...
        if experimental:
            ...
            for group, axes in zip(groups, ax.flat):
                nan_values, upper_lim, _ = self.compute_missing_values_outlier(
                    data[group]
                )
